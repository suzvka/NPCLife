...
### 1. 核心组件与模式
*   **ErrorHandler (全局中枢)**：位于 `src/NPCLife/Framework/ErrorHandler.cs`。提供纯静态的错误报告接口 `ReportError`。它不直接抛出异常，而是将错误封装为 `ErrorContext` 并分发给注册的处理器。支持**诊断模式**（DiagnosticMode）以输出详细日志。
