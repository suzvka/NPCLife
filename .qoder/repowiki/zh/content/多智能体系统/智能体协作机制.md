...
## 更新摘要
**变更内容**
- 更新了AgentLoop组件以使用新的ICredentialStore接口
- 新增了模型引用功能，支持更灵活的凭证解析机制
- 增强了凭证解析的优先级和回退机制
- 添加了模型引用列表和当前选中模型的配置支持
...
**更新** 本版本新增了基于ICredentialStore接口的凭证解析机制，支持模型引用功能，提供了更灵活的API凭证管理和模型选择能力。

## 项目结构
围绕"工作空间 + 事件池 + 智能体循环 + 凭证管理"的核心设计，系统采用分层组织：
- Workspace 层：工作空间状态、事件池、技能槽、角色与状态定义
- Agent 层：智能体循环（AgentLoop）与管道拦截（AgentPipeline）
- Framework 层：事件总线（EventBus）、提示词与工具集成
- Core 层：凭证存储接口（ICredentialStore）和凭证管理实现
...
subgraph "凭证管理层"
ICredStore["ICredentialStore<br/>凭证存储接口"]
CredRegistry["CredentialRegistry<br/>凭证注册表实现"]
LlmCred["LlmCredential<br/>LLM凭证"]
end
subgraph "框架层"
...
Agent --> Pipe
Agent --> ICredStore
ICredStore --> CredRegistry
CredRegistry --> LlmCred
Agent --> Bus
WSM --> Bus
WS --> Bus
...
- [ICredentialStore.cs:11-31](file://src/NPCLife/Core/ICredentialStore.cs#L11-L31)
- [CredentialRegistry.cs:19-91](file://src/NPCLife/Infrastructure/Llm/CredentialRegistry.cs#L19-L91)
...
- 工作空间（Workspace）
  ...
  - WorkspaceState：工作空间状态与轮次日志的数据载体，包含模型引用和当前模型配置
  - WorkspaceEventPool：事件池，支持阈值触发、最近历史缓存、DrainPending 消费
  - SkillSlot：技能槽，封装激活/停用技能与工具集合
  - RoleSkillProfile：按角色预设默认技能集
- 智能体（Agent）
  - AgentLoop：事件驱动的智能体循环，状态机驱动，支持 LLM + 工具调用，集成ICredentialStore接口
  - AgentPipeline：拦截器管道，贯穿 Prompt/Llm/ToolCall/Finish 阶段
- 凭证管理（Credential Management）
  - ICredentialStore：凭证存储接口，提供运行时凭证访问能力
  - CredentialRegistry：凭证注册表实现，管理凭证创建、更新、删除和激活顺序
  - LlmCredential：LLM API凭证数据类，包含基础URL、API密钥、模型名称等信息
- 事件总线（EventBus）：发布/订阅，命名空间事件名，错误隔离与优先级排序
- 角色提示词：DirectorPrompt、ScreenwriterPrompt、FreelancerPrompt

**更新** 新增了凭证管理层次，通过ICredentialStore接口提供灵活的凭证解析机制，支持模型引用功能。
...
## 架构总览
多智能体协作以"事件驱动 + 工作空间隔离 + 管道拦截 + 凭证管理"为核心：
- Director：全局视角，负责事件路由、工作空间创建/分支/合并、状态变更
- Screenwriter：叙事创作，基于事件池与工具集产出台词与轮次，支持模型引用配置
- Freelancer：独立事件快速响应，无需跨轮次上下文
- 事件池阈值触发智能体循环，EventBus 贯穿生命周期事件，AgentPipeline 注入拦截逻辑
- 凭证管理：通过ICredentialStore接口提供灵活的凭证解析和模型选择能力
...
participant CredStore as "ICredentialStore"
participant CredRegistry as "CredentialRegistry"
...
Agent->>Agent : "ResolveCredentials()<br/>解析模型引用"
Agent->>CredStore : "Resolve(凭证名, 模型名)"
CredStore->>CredRegistry : "查找并克隆凭证"
CredRegistry-->>Agent : "返回LlmCredential"
...
**更新** 新增了凭证解析流程，AgentLoop通过ICredentialStore接口解析模型引用，支持优先级和回退机制。
...
- [ICredentialStore.cs:17-29](file://src/NPCLife/Core/ICredentialStore.cs#L17-L29)
- [CredentialRegistry.cs:84-91](file://src/NPCLife/Infrastructure/Llm/CredentialRegistry.cs#L84-91)
...
### 工作空间与事件池
- WorkspaceState：承载工作空间元数据、轮次日志、当前前情提要、事件缓存与待处理事件队列，包含模型引用和当前模型配置
...
- WorkspaceManager：CRUD、分支/合并、事件路由、持久化与状态变更广播，支持模型引用配置序列化
...
Drain --> ResolveCred["ResolveCredentials()<br/>解析模型引用和凭证"]
ResolveCred --> Done(["进入智能体循环"])
```

**更新** 新增了凭证解析步骤，AgentLoop在进入智能体循环前解析模型引用配置。
...
### 智能体循环与管道拦截
- AgentLoop：显式状态机（Idle/DrainingEvents/BuildingRequest/CallingLlm/ExecutingTools/AppendingToolResults/Finishing/Error），被动激活（OnThresholdReached），统一成功/失败路径，集成ICredentialStore接口
- AgentPipeline：拦截器链（BeforePrompt/BeforeLlm/BeforeToolCall/AfterToolCall/LoopFinished），支持优先级与取消
- 凭证解析：支持模型引用列表和当前选中模型，提供优先级和回退机制
...
Agent->>Agent : "ResolveCredentials()<br/>解析模型引用"
Agent->>CredStore : "Resolve(凭证名, 模型名)"
CredStore-->>Agent : "返回LlmCredential"
...
**图表来源**
- [AgentLoop.cs:171-337](file://src/NPCLife/Agent/AgentLoop.cs#L171-L337)
- [AgentLoop.cs:208-211](file://src/NPCLife/Agent/AgentLoop.cs#L208-L211)
- [AgentLoop.cs:569-607](file://src/NPCLife/Agent/AgentLoop.cs#L569-L607)
- [ICredentialStore.cs:25-29](file://src/NPCLife/Core/ICredentialStore.cs#L25-L29)
- [AgentPipeline.cs:180-236](file://src/NPCLife/Framework/AgentPipeline.cs#L180-L236)
...
### 凭证管理与模型引用
- ICredentialStore：凭证存储接口，提供运行时凭证访问能力，支持凭证解析和可用性检查
- CredentialRegistry：凭证注册表实现，管理凭证创建、更新、删除、激活顺序和模型设置
- LlmCredential：LLM API凭证数据类，包含基础URL、API密钥、模型名称、提供商类型等信息
- 模型引用：支持JSON格式的模型引用列表，包含凭证名和模型名，提供优先级和回退机制
- 凭证解析：优先使用模型引用列表，回退到全局激活凭证，当前选中模型优先排列

```mermaid
flowchart TD
Start(["ResolveCredentials"]) --> CheckRefs{"是否有模型引用?"}
CheckRefs --> |是| ParseRefs["解析模型引用列表<br/>[{cred, model}, ...]"]
ParseRefs --> ParseCurrent["解析当前选中模型<br/>{cred, model}"]
ParseRefs --> ResolveEach["逐个解析凭证<br/>credentialStore.Resolve(cred, model)"]
ResolveEach --> CheckReady{"凭证是否就绪?"}
CheckReady --> |是| AddToResult["添加到结果列表"]
CheckReady --> |否| Skip["跳过该凭证"]
AddToResult --> CheckCurrent{"是否为当前选中模型?"}
CheckCurrent --> |是| MoveToFront["移动到列表首位"]
CheckCurrent --> |否| Continue["继续下一个"]
MoveToFront --> ReturnResult["返回解析结果"]
Skip --> Continue
Continue --> CheckAllDone{"所有凭证都处理完?"}
CheckAllDone --> |否| ResolveEach
CheckAllDone --> |是| ReturnResult
CheckRefs --> |否| UseFallback["使用全局激活凭证<br/>credentialStore.GetActiveCredentials()"]
UseFallback --> ReturnFallback["返回全局凭证列表"]
```

**更新** 新增了完整的凭证解析流程，支持模型引用列表和当前选中模型的优先级机制。
...
- [CredentialRegistry.cs:19-91](file://src/NPCLife/Infrastructure/Llm/CredentialRegistry.cs#L19-L91)
- [LlmCredential.cs:12-84](file://src/NPCLife/Framework/Llm/LlmCredential.cs#L12-L84)
- [AgentLoop.cs:569-655](file://src/NPCLife/Agent/AgentLoop.cs#L569-L655)
...
- Screenwriter（编剧）
  ...
  - 支持模型引用配置，可根据工作空间设置选择不同的API模型
- Freelancer（临时工）
  - 职责：处理突发性、独立事件，无需跨轮次上下文，快速输出轮次
  - 角色提示词强调"独立任务、快速响应、不维护延续性"

**更新** 编剧角色现在支持模型引用配置，可以根据工作空间设置选择不同的API模型。
...
**更新** 新增了凭证状态同步机制，通过持久化委托和事件总线传播凭证状态变化。
...
**更新** 工作空间状态现在包含模型引用和当前模型配置，支持更灵活的模型选择。
...
- 场景四：凭证管理与模型选择
  - 通过ICredentialStore接口管理多个API凭证
  - 使用模型引用列表实现灵活的模型选择和优先级控制
  - 参考路径：[ICredentialStore.cs:11-31](file://src/NPCLife/Core/ICredentialStore.cs#L11-L31)、[CredentialRegistry.cs:19-91](file://src/NPCLife/Infrastructure/Llm/CredentialRegistry.cs#L19-L91)

**更新** 新增了凭证管理和模型选择的最佳实践场景。

## 依赖分析
- 组件耦合
  - WorkspaceImpl 依赖 WorkspaceState、WorkspaceEventPool、SkillSlot、EventBus
  - AgentLoop 依赖 IEventLog（事件池）、ILlmService、ICredentialStore、AgentPipeline、EventBus
  - WorkspaceManager 依赖 IAuthorityStore、ILogger、DriverConfig、ICardSerializer、EventBus
  - ICredentialStore 依赖 LlmCredential 数据结构
  - CredentialRegistry 实现 ICredentialStore 接口
- 外部依赖
  - 事件序列化：ICardSerializer（CardSerializer.Default）
  - 工具调用：McpSkillRegistry（通过 AgentLoop 间接使用）
  - 凭证持久化：通过持久化委托保持状态一致性
- 潜在循环依赖
  - 通过接口解耦（IEventLog、IWorkspace、IWorkspaceManager、ICredentialStore），未见循环
...
Agent --> ICredStore["ICredentialStore"]
ICredStore --> CredRegistry["CredentialRegistry"]
CredRegistry --> LlmCred["LlmCredential"]
Agent --> Bus["EventBus"]
WSM --> Bus
WS --> Bus
```
...
- 凭证解析失败
  - 检查模型引用JSON格式是否正确
  - 验证凭证名称和模型名称是否存在于凭证注册表中
  - 确认凭证是否处于就绪状态（HasApiAccess/IsChatReady）
  - 参考路径：[AgentLoop.cs:569-607](file://src/NPCLife/Agent/AgentLoop.cs#L569-L607)、[ICredentialStore.cs:17-29](file://src/NPCLife/Core/ICredentialStore.cs#L17-L29)

**更新** 新增了凭证解析相关的故障排查指南。

## 结论
该多智能体协作体系以"事件池阈值触发 + 工作空间隔离 + 管道拦截 + 凭证管理"为核心，实现了 Director/Screenwriter/Freelancer 的清晰分工与高效协同。通过 EventBus 与 AgentPipeline，系统在保证可观测性与可扩展性的同时，兼顾了性能与稳定性。

**更新** 新版本通过ICredentialStore接口和模型引用功能，提供了更灵活的凭证解析机制，支持多模型、多凭证的灵活配置和优先级控制，增强了系统的可扩展性和实用性。

建议在实际部署中：
- 依据业务特征调整角色阈值与最近历史容量
- 按需引入拦截器，避免不必要的开销
- 健全监控与告警，结合事件总线定位问题
- 合理配置模型引用列表，实现灵活的API模型选择
- 通过凭证注册表管理多个API凭证，实现负载均衡和故障转移
...
- 凭证管理参考
  - ICredentialStore：凭证存储接口，提供运行时凭证访问能力
  - CredentialRegistry：凭证注册表实现，支持CRUD操作和激活顺序管理
  - LlmCredential：LLM API凭证数据类，包含基础URL、API密钥、模型名称等信息
