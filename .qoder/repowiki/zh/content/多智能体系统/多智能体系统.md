...
## 更新摘要
**所做更改**
- 更新了智能体系统架构章节，反映 AgentLoop 构造函数的重大简化
- 新增了依赖注入模式的详细说明
- 更新了相关代码示例和架构图
- 补充了新的依赖注入模式对系统架构的影响分析
...
**更新** 本版本重点反映了 AgentLoop 构造函数的重大简化，从原来的 14 个参数减少到 4 个参数，引入了新的依赖注入模式，显著提升了系统的可维护性和扩展性。
...
subgraph "智能体层"
AL["AgentLoop"]
ALD["AgentLoopDependencies 依赖注入"]
...
- [AgentLoopDependencies.cs:11-51](file://src/NPCLife/Agent/AgentLoopDependencies.cs#L11-L51)
...
- **新增** 依赖注入（AgentLoopDependencies）：集中管理 AgentLoop 的基础设施依赖和行为配置，替代原有的 14 个构造函数参数。
...
**更新** 现在的 AgentLoop 构造函数通过依赖注入模式简化为 4 个参数，大大降低了使用复杂度。
...
participant Deps as "AgentLoopDependencies"
...
- [AgentLoopDependencies.cs:34-49](file://src/NPCLife/Agent/AgentLoopDependencies.cs#L34-L49)
...
- **更新** 构造函数简化
  - 从原来的 14 个参数简化为 4 个参数：workspace、deps、systemPrompt、contextProvider。
  - deps 参数集中管理所有基础设施依赖，大幅降低构造复杂度。
- 请求构建与 LLM 调用
...
### AgentLoopDependencies 架构设计
**新增** 为了应对 AgentLoop 构造函数参数过多的问题，引入了 AgentLoopDependencies 结构体来集中管理所有基础设施依赖：

- **基础设施依赖**
  - ILlmService：LLM 异步对话服务（必填）
  - ICredentialStore：凭证存储（必填）
  - ILogger：日志接口（必填）
  - ICardSerializer：Card 序列化器（可选，默认 CardSerializer.Default）

- **行为配置**
  - MaxRounds：Agent 多轮工具调用最大轮数（默认 10）
  - Temperature：LLM 采样温度（默认 0.7）

- **默认配置工厂**
  - CreateDefault 方法提供生产环境的标准配置
  - 宿主只需提供 Llm、CredentialStore、Logger 三个必需依赖即可
...
public AgentLoop(
    ...
    string currentModel,
    string workspaceId,
    Func<string> dynamicContextProvider)
```
...
### 依赖注入优势
- **简化使用**：从 14 个参数减少到 4 个参数，大幅降低使用复杂度
- **集中管理**：所有依赖集中在 AgentLoopDependencies 中，便于测试和配置
- **默认值支持**：提供合理的默认值，减少样板代码
- **向后兼容**：保持相同的功能和行为，不影响现有代码

**章节来源**
- [AgentLoop.cs:81-104](file://src/NPCLife/Agent/AgentLoop.cs#L81-L104)
- [AgentLoopDependencies.cs:11-51](file://src/NPCLife/Agent/AgentLoopDependencies.cs#L11-L51)

## 依赖分析
- 组件耦合
  - AgentLoop 依赖 IEventLog（WorkspaceEventPool 实现）、ILlmService、ICredentialRegistry、IKnowledgeService、ILogger、McpSkillRegistry。
  - **更新** 现在通过 AgentLoopDependencies 集中管理依赖，减少了直接的外部依赖暴露。
...
ALD --> LLM["ILlmService"]
ALD --> CR["ICredentialStore"]
ALD --> LOG["ILogger"]
ALD --> SER["ICardSerializer"]
...
- AgentLoop 常见问题
  ...
  - **新增** 依赖注入问题：检查 AgentLoopDependencies 配置是否正确设置。
- 工作空间问题
...
## 结论
该多智能体系统通过"事件驱动 + 工具调用"的模式实现了导演、编剧与临时编剧的清晰分工与高效协作。导演负责结构化管理与事件路由，编剧专注于叙事内容与上下文沉淀，临时编剧处理突发与独立事件。事件总线与工作空间状态机确保了异步消息传递与状态一致性，生命周期管理提供了稳健的启动/销毁/重置能力。

**更新** 最重要的改进是 AgentLoop 构造函数的重大简化，从 14 个参数减少到 4 个参数，引入了 AgentLoopDependencies 依赖注入模式。这一变化显著提升了系统的可维护性、可测试性和扩展性，同时保持了原有的功能完整性。通过阈值控制、并行工具调用与拦截器扩展，系统在性能与可扩展性之间取得良好平衡。
...
- **更新** 代码示例参考路径
  - AgentLoop 构造函数简化：[AgentLoop.cs:81-104](file://src/NPCLife/Agent/AgentLoop.cs#L81-L104)
  - 依赖注入配置：[AgentLoopDependencies.cs:34-49](file://src/NPCLife/Agent/AgentLoopDependencies.cs#L34-L49)
