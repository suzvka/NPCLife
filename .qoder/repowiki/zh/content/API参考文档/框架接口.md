...
## 更新摘要
**所做更改**
- 新增框架服务接口抽象层：IEventBus、IAgentPipeline、IMetricsRecorder、IFrameworkStatus、IMcpSkillRegistry
- 添加相应的静态适配器实现用于向后兼容
- 扩展框架接口文档以涵盖新的基础设施服务
- 更新架构总览以反映新增的服务接口
...
- 接口层：IScriptConsumer（脚本消费者）、IScriptLineResolver（脚本行解析器）、IEventBus（事件总线）、IAgentPipeline（代理管道）、IMetricsRecorder（度量记录器）、IFrameworkStatus（框架状态）、IMcpSkillRegistry（MCP技能注册表）
...
## 项目结构
围绕"接口 + 工具 + 服务接口 + 适配器实现"的组织方式：
- Core：定义领域接口与数据契约
- Framework：提供 JSON 转义、解析、写入等轻量工具，以及新的服务接口抽象
- Infrastructure：提供静态适配器实现，将静态服务转换为可注入实例
...
subgraph "服务接口(Framework)"
D["IEventBus<br/>事件总线接口"]
E["IAgentPipeline<br/>代理管道接口"]
F["IMetricsRecorder<br/>度量记录器接口"]
G["IFrameworkStatus<br/>框架状态接口"]
H["IMcpSkillRegistry<br/>MCP技能注册表接口"]
end
subgraph "框架工具(Framework)"
I["JsonHelper<br/>转义/引用"]
J["JsonParser<br/>解析/序列化"]
K["JsonWriter<br/>写入器"]
end
subgraph "适配器实现(Infrastructure)"
L["StaticEventBusAdapter<br/>事件总线适配器"]
M["StaticAgentPipelineAdapter<br/>代理管道适配器"]
N["StaticMetricsRecorderAdapter<br/>度量记录器适配器"]
O["StaticFrameworkStatusAdapter<br/>框架状态适配器"]
P["StaticMcpSkillRegistryAdapter<br/>MCP技能注册表适配器"]
end
subgraph "基础设施(Infrastructure)"
Q["ScriptDeliveryService<br/>台词推送服务"]
R["BuiltInKnowledgeBase<br/>内置知识库"]
end
subgraph "测试(Test)"
T1["JsonHelperTests"]
T2["JsonParserTests"]
...
**图表来源**
- [IEventBus.cs:9-25](file://src/NPCLife/Framework/IEventBus.cs#L9-L25)
- [IAgentPipeline.cs:6-13](file://src/NPCLife/Framework/IAgentPipeline.cs#L6-L13)
- [IMetricsRecorder.cs:6-22](file://src/NPCLife/Framework/IMetricsRecorder.cs#L6-L22)
- [IFrameworkStatus.cs:9-22](file://src/NPCLife/Framework/IFrameworkStatus.cs#L9-L22)
- [IMcpSkillRegistry.cs:10-38](file://src/NPCLife/Framework/Mcp/IMcpSkillRegistry.cs#L10-L38)
- [StaticEventBusAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticEventBusAdapter.cs#L1-L200)
- [StaticAgentPipelineAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticAgentPipelineAdapter.cs#L1-L200)
- [StaticMetricsRecorderAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticMetricsRecorderAdapter.cs#L1-L200)
- [StaticFrameworkStatusAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticFrameworkStatusAdapter.cs#L1-L200)
- [StaticMcpSkillRegistryAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticMcpSkillRegistryAdapter.cs#L1-L200)
...
### 服务接口层
- IEventBus（事件总线接口）
  - 职责：将静态 EventBus 抽象为可注入实例，提供事件发布、订阅和管理功能
  - 方法定义与参数
    - Publish(eventName, args = null)：发布事件
    - Subscribe(eventName, handler, priority = 0)：订阅事件，返回取消订阅委托
    - ClearAll()：清除全部订阅
    - Logger：日志接口属性注入
  - 优先级机制：priority 越小越先执行（0 为默认）
  - 章节来源
    - [IEventBus.cs:11-24](file://src/NPCLife/Framework/IEventBus.cs#L11-L24)

- IAgentPipeline（代理管道接口）
  - 职责：管理代理拦截器，提供管道拦截功能
  - 方法定义与参数
    - AddInterceptor(interceptor, priority = 0)：添加拦截器
    - Logger：日志接口属性注入
  - 章节来源
    - [IAgentPipeline.cs:8-12](file://src/NPCLife/Framework/IAgentPipeline.cs#L8-L12)

- IMetricsRecorder（度量记录器接口）
  - 职责：记录框架运行时度量指标
  - 方法定义与参数
    - RecordTokenUsage(sessionId, inputTokens, outputTokens, cacheReadTokens, model)：记录LLM请求Token消耗
    - RecordWorkspaceOperation(operation)：记录工作空间操作
    - RecordKnowledgeLookup(term, hitLayer, sessionId)：记录知识库查询
    - GetSnapshot()：获取度量快照
    - Reset()：重置所有度量
  - 章节来源
    - [IMetricsRecorder.cs:8-21](file://src/NPCLife/Framework/IMetricsRecorder.cs#L8-L21)

- IFrameworkStatus（框架状态接口）
  - 职责：提供框架状态内省和健康检查功能
  - 方法定义与参数
    - RegisterReporter(componentName, reporter)：注册组件状态报告器
    - RegisterCapability(capabilityName, supported)：注册能力标识
    - HealthCheck()：健康检查
    - Clear()：清除所有报告器和能力注册
  - 章节来源
    - [IFrameworkStatus.cs:11-21](file://src/NPCLife/Framework/IFrameworkStatus.cs#L11-L21)

- IMcpSkillRegistry（MCP技能注册表接口）
  - 职责：管理MCP技能和工具的注册与调用
  - 方法定义与参数
    - InitializeDefaults()：初始化默认技能元数据
    - RegisterFromProvider(provider)：从提供者注册工具
    - SkillCount：已注册技能数
    - TotalToolCount：工具总数
    - GetSkillListJson(activeSkillIds)：获取技能列表JSON
    - GetActiveToolsJson(activeSkillIds)：获取激活技能工具JSON
    - GetAllSkillIds()：获取所有技能ID列表
    - GetSkillToolsJson(skillId)：获取指定技能工具JSON
    - InvokeTool(activeSkillIds, toolName, jsonArgs)：调用指定工具
  - 章节来源
    - [IMcpSkillRegistry.cs:12-37](file://src/NPCLife/Framework/Mcp/IMcpSkillRegistry.cs#L12-L37)
...
### 服务接口：IEventBus、IAgentPipeline、IMetricsRecorder、IFrameworkStatus、IMcpSkillRegistry
- 设计要点
  - 接口抽象：将静态服务转换为可注入实例，提高可测试性和可替换性
  - 依赖注入：通过属性注入 ILogger，支持日志配置
  - 统一管理：提供 ClearAll、Clear 等清理方法，便于资源管理
- 使用流程
  - 事件总线：Subscribe 返回 Action 用于取消订阅，支持优先级排序
  - 度量记录：Record* 方法提供细粒度的性能监控
  - 状态管理：RegisterReporter 和 RegisterCapability 支持动态状态报告
  - 技能注册：InitializeDefaults 和 RegisterFromProvider 支持批量技能管理
- 错误处理
  - 适配器实现提供防御性检查和日志记录
  - 支持部分失败场景，不影响整体框架运行
- 章节来源
...
- 适配器实现
  - StaticEventBusAdapter：将静态 EventBus 包装为可注入实例
  - StaticAgentPipelineAdapter：将静态 AgentPipeline 包装为可注入实例
  - StaticMetricsRecorderAdapter：将静态 RuntimeMetrics 包装为可注入实例
  - StaticFrameworkStatusAdapter：将静态 FrameworkStatus 包装为可注入实例
  - StaticMcpSkillRegistryAdapter：将静态 McpSkillRegistry 包装为可注入实例
  - 章节来源
    - [StaticEventBusAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticEventBusAdapter.cs#L1-L200)
    - [StaticAgentPipelineAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticAgentPipelineAdapter.cs#L1-L200)
    - [StaticMetricsRecorderAdapter.cs:1-200](file://src/NPCLife/Infrastructure/StaticMetricsRecorderAdapter.cs#L1-L200)
...
StaticEventBusAdapter --> EventBus
StaticAgentPipelineAdapter --> AgentPipeline
StaticMetricsRecorderAdapter --> RuntimeMetrics
StaticFrameworkStatusAdapter --> FrameworkStatus
StaticMcpSkillRegistryAdapter --> McpSkillRegistry
```
...
## 性能考量
- 服务接口层
  - 零依赖注入：通过属性注入 ILogger，避免构造函数复杂性
  - 优先级排序：事件和拦截器支持优先级，确保执行顺序可控
  - 快照机制：IMetricsRecorder 提供 GetSnapshot 支持非阻塞状态读取
...
## 结论
IScriptConsumer 与 IScriptLineResolver 为框架提供了稳定的台词分发与解析契约；新增的服务接口（IEventBus、IAgentPipeline、IMetricsRecorder、IFrameworkStatus、IMcpSkillRegistry）通过静态适配器实现了向后兼容，同时提供了更好的可测试性和可扩展性。JsonHelper/JsonParser/JsonWriter 则以零依赖、高性能的方式支撑了 JSON 的读写需求。结合 ScriptDeliveryService 与 BuiltInKnowledgeBase 的实际实现，可以高效地完成从事件到展示的闭环，并具备良好的可扩展性与可维护性。
...
- 扩展与自定义指导
  ...
  - 自定义服务接口：实现对应接口并提供适配器，确保与现有静态服务的兼容性
- 集成步骤建议
